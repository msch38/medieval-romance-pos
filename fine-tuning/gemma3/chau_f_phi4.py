...
change model name and dataset according to the needs
"""
...
# Read sentences
with open("Chauliac_reference.txt", "r") as f:
    sentences = [line.strip().split() for line in f]

# Read POS tags
df = pd.read_excel("Chauliac_reference.xlsx")
all_tags = []
invalid_tags = set()
...
# Tokenizer and model
model_name="microsoft/phi-4" #model
...
with open("chauliac_cat_naf/tag2id_chau.json", "w") as f:
    json.dump(tag2id, f)
with open("chauliac_cat_naf/classification_report_chau_f_phi4.txt", "w") as f:
    f.write(report)